...
{	
	...
	Simulator(std::ifstream &file,bool forwarding_state)
	{
		instructions = {{"add", &Simulator::add}, {"sub", &Simulator::sub},{"and", &Simulator::And}, {"or", &Simulator::Or},
                ...
                {"ori", &Simulator::ori},{"rem",&Simulator::rem}};
	...
	int rem(std::string r1, std::string r2, std::string r3)
	{
		if (!checkRegisters({r1, r2, r3}) || registerMap[r1] == 0){
			return 1;
		}
		if(registers[registerMap[r3]]==0){
			registers[registerMap[r1]] = registers[registerMap[r2]];
		}
		else{
	...
	int perform_op(std::string r1, std::string r2, std::string branch_operand, std::function<bool(int, int)> comp)
	{
		if (!checkRegisters({r1, r2}))
			...
			// interpreting branch_operand as an immediate offset.
	...
	int jal(std::string rd, std::string offset_str,std::string unused)
	{
	...
	void executeCommands()
	{
		...
		while (PCcurr < commands.size())
		{
			...
			pipeline->run_command(parser->parametric_commands[PCcurr], cmd_str); // Pass the command string
			pipeline->save();
